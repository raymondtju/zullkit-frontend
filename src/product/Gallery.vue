<script setup>
import { ref } from 'vue'
const defaultGal = ref("gallery-3.png")
const galleries = ref([
...
function changeImage(image){
    this.defaultGal = image
}
</script>

<template>
    <section id="gallery">
        <img :src="'/src/assets/img/' + defaultGal" alt="" class="w-full mt-6 rounded-2xl">
        <div class="grid grid-cols-4 gap-4 mt-4">
            <template v-for="gallery in galleries" :key="gallery.id">
                <div @click="changeImage(gallery.image)" class="overflow-hidden cursor-pointer rounded-2xl" :class="{'ring-2 ring-indigo-500' : defaultGal == gallery.image}">
